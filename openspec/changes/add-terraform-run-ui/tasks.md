...
- [x] 5.9 Open PR via the `/pr` command with a Conventional Commits title (`feat(desktop): guarded terraform destroy IPC + UI`, <70 chars); PR body's FIRST line must be `Closes #307`, and — as this PR completes the last open issue of epic #138 — the body must also include `Closes #138` on its own line so the epic auto-closes
